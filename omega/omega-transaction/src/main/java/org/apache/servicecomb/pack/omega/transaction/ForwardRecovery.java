...
/**
 * ForwardRecovery is used to execute business logic with the given forwardRetries times. If forwardRetries is above 0,
 * it will retry the given times at most. If forwardRetries == -1, it will retry forever until interrupted.
 */
public class ForwardRecovery extends DefaultRecovery {
    ...
    public Object applyTo(ProceedingJoinPoint joinPoint, Compensable compensable, CompensableInterceptor interceptor,
        ...
        try {
            // servicecomb 向前恢复的策略：是指在重试次数条件下，如果异常，则进行重复发起调用
            // 而不是由 alpha 来通知进行整体事务的重放, 好处是
            while (true) {
                try {
